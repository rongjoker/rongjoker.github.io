...
const states = {
  ...
  start: () => {
    ...
    store.dispatch(actions.combo(0));
  ...
  nextAround: (matrix, stopDownTrigger) => {
    ...
    if (isClear(matrix)) {
      store.dispatch(actions.combo(store.getState().get('combo') + 1));
      if (music.clear) {
        music.clear();
      }
      return;
  ...
  clearLines: (matrix, lines) => {
    ...
    const comboBonus = Math.max(0, state.get('combo') - 1) * 50;
    const addPoints = store.getState().get('points') +
      clearPoints[lines.length - 1] + comboBonus;
    states.dispatchPoints(addPoints);
