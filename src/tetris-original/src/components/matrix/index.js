...
import { isClear, landing } from '../../unit/';
...
export default class Matrix extends React.Component {
  ...
  getResult(props = this.props) {
    ...
    if (clearLines) {
      ...
      if (!props.reset) {
        const ghost = landing(cur, matrix);
        ghost.shape.forEach((m, k1) => (
          m.forEach((n, k2) => {
            const y = ghost.xy[0] + k1;
            const x = ghost.xy[1] + k2;
            if (n && y >= 0 && y < 20 && matrix.get(y).get(x) === 0) {
              matrix = matrix.set(y, matrix.get(y).set(x, 3));
            }
          })
        ));
      }
  ...
  render() {
    ...
    return (
      <div className={style.matrix}>{
        ...
        {this.state.clearLines && this.props.combo > 1 ?
          <span className={style.combo}>{`COMBO ×${this.props.combo}`}</span> : null}
      </div>
    );
  }
}
